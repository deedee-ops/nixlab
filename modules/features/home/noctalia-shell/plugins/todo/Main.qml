...
Item {
  ...
  property bool isWritingFile: false
  property bool pendingWrite: false
  property bool fileErrorShown: false
  ...
  // Process for reading the todo sync file
  Process {
    id: fileReadProcess
    running: false
    stdout: SplitParser {
    ...
    onExited: function (code) {
      ...
      if (code !== 0) {
        showFileError("Could not read todo file (exit " + code + ")");
        return;
      }
      try {
        ...
        showFileError("Todo file contains invalid JSON: " + e.message);
      }
    }
  }

  // Process for writing the todo sync file (atomic: write to .tmp then mv)
  Process {
    id: fileWriteProcess
    running: false
    onExited: function (code) {
      isWritingFile = false;
      if (code !== 0)
        showFileError("Could not write todo file (exit " + code + ")");
      if (pendingWrite) {
        pendingWrite = false;
        writeTodosFile();
      }
  ...
  // Timer for polling the sync file for external changes (e.g. from Syncthing)
  Timer {
    id: filePollingTimer
    interval: 30000
    repeat: true
  ...
  Component.onCompleted: {
    if (pluginApi) {
      ...
      if (!pluginApi.pluginSettings.todoFilePath)
        pluginApi.pluginSettings.todoFilePath = pluginApi.manifest.metadata.defaultSettings.todoFilePath;

      // Initialize current page if missing
      if (pluginApi.pluginSettings.current_page_id === undefined)
        pluginApi.pluginSettings.current_page_id = 0;

      pluginApi.saveSettings();

      // Load todos and pages from sync file; start polling for external changes
      readTodosFile();
      filePollingTimer.start();
    }
  }
  ...
  function saveTodos() {
    if (!pluginApi || !pluginApi.pluginSettings)
      return;
    // Mirror to pluginSettings so Panel/DesktopWidget bindings fire
  ...
  function savePages() {
    if (!pluginApi || !pluginApi.pluginSettings)
      return;
    // Mirror to pluginSettings so Settings.qml Repeater stays reactive
  ...
  function readTodosFile() {
    if (!pluginApi)
      return;
    var path = resolveFilePath(pluginApi.pluginSettings.todoFilePath);
    if (!path)
      return;
    fileReadProcess.command = ["sh", "-c", `[ -f "${path}" ] && cat "${path}" || exit 2`];
    fileReadProcess.running = true;
  }

  function writeTodosFile() {
    if (!pluginApi)
      return;
    if (isWritingFile) {
      pendingWrite = true;
    ...
    var data = {
      version: 1,
      todos: rawTodos.slice(),
      pages: rawPages.slice()
    };
    var base64 = Qt.btoa(JSON.stringify(data));
    isWritingFile = true;
    fileWriteProcess.command = ["sh", "-c", `printf '%s' "${base64}" | base64 -d > "${path}.tmp" && mv "${path}.tmp" "${path}"`];
    fileWriteProcess.running = true;
  }

  function createTodosFile() {
    rawTodos = [];
    rawPages = [{id: 0, name: "General"}];
  ...
  function applyFileData(data) {
    if (!Array.isArray(data.todos) || !Array.isArray(data.pages)) {
      showFileError("Todo file has invalid structure");
      return;
    }
    // Data migration: add missing fields
    var todos = data.todos.slice();
    ...
    rawTodos = todos;
    rawPages = data.pages.slice();
    // Mirror to pluginSettings for UI reactivity
    pluginApi.pluginSettings.todos = rawTodos.slice();
    pluginApi.pluginSettings.pages = rawPages.slice();
    pluginApi.pluginSettings.count = rawTodos.length;
    pluginApi.pluginSettings.completedCount = rawTodos.filter(t => t.completed).length;
  ...
  function reloadTodosFile() {
    filePollingTimer.stop();
    fileErrorShown = false;
    readTodosFile();
    filePollingTimer.start();
  }
