...
ColumnLayout {
  ...
  // Sync file path property
  property string valueTodoFilePath: pluginApi?.pluginSettings?.todoFilePath !== undefined ? pluginApi.pluginSettings.todoFilePath : (pluginApi?.manifest?.metadata?.defaultSettings?.todoFilePath ?? "~/Sync/sync/noctalia/todo.json")
  ...
  // Sync file path setting
  NTextInputButton {
    Layout.fillWidth: true
    label: "Sync File Path"
    description: "Path to the JSON file shared via Syncthing (or any sync tool). Each machine can point to its own folder."
    placeholderText: "~/Sync/sync/noctalia/todo.json"
    text: root.valueTodoFilePath
    buttonIcon: "folder-open"
    buttonTooltip: pluginApi?.tr("settings.export_path.select_folder")
    onInputEditingFinished: {
      root.valueTodoFilePath = text;
      if (mainInstance && mainInstance.pluginApi) {
        mainInstance.pluginApi.pluginSettings.todoFilePath = text;
        mainInstance.pluginApi.saveSettings();
        mainInstance.reloadTodosFile();
      }
    }
    onButtonClicked: todoFilePicker.openFilePicker()
  }

  NFilePicker {
    id: todoFilePicker
    selectionMode: "files"
    title: "Select Todo Sync File"
    initialPath: root.valueTodoFilePath
    onAccepted: function (paths) {
      if (paths.length > 0) {
        root.valueTodoFilePath = paths[0];
        mainInstance.pluginApi.pluginSettings.todoFilePath = paths[0];
        mainInstance.pluginApi.saveSettings();
        mainInstance.reloadTodosFile();
      }
    }
